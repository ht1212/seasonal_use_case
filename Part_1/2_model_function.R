...
run_sites <- function(run, pfpr, vectors, seasonality, seasonality_coefs, draw, 
  ...
  #-turning the antibody model on and setting boost mu parameter as we're modelling differently for each run------
  if(run %in% c("srtss_4_dose_update", "srtss_5_dose_update")){
  ...
  options = paste("recalculate 2 num_people 100000 final_run 20 num_runs 3 itn 0 irs 0 smc 0 output_type 0 drug_cov_0_0 0.26470592 drug_cov_1_0 0.2647059 drug_cov_2_0 0.0000000 drug_cov_3_0 0.0000000",
  ...
  #-altered local package to be able to set model file location-----------------------------------------------------
  mf_loc <- paste0("Q:/who_use_case/validation/model_files/0/model_files")
  ...
  # Save output
  output_address <- paste0(run, "/output/", name, ".csv")
