#-Script 1-------------------------------------------------------------------------------------- 
# creating the data frame of parameter values to pass to the malaria function
...
#-output function from Pete's work to make this easier and aligned-------------------------------
...
run <- c("epi", # age based vaccination
...
#-coverage values--------------------------------------------------------------------------------
coverage <- 0.8 #seq(0.5, 0.9, 0.02)

#-create initial dataframe-----------------------------------------------------------------------
vaccine_df <- crossing(run, pfpr, seasonality, draw, coverage) 
baseline <- data.frame(run = "baseline", coverage = 0) %>% crossing(pfpr, seasonality, draw)

#-add additional run specific parameters---------------------------------------------------------
param_df <-
  bind_rows(vaccine_df, baseline) %>% 
  mutate(epi_pev = case_when(run == "epi" ~ 1, TRUE ~ 0), 
...
rm(baseline, df1, param_df, vaccine_df, coverage, draw, pfpr, run, seasonality)
...
source("s2_model_function.R")
system.time({out <- future_pmap(median_df[3,], run_sites, .progress = TRUE)})