...
intervention_df <- crossing(run, pfpr, seasonality, draw, coverage) 
smc             <- data.frame(run = "smc", coverage = 0) %>% crossing(pfpr, seasonality, draw)
...
param_df <-
  ...
  mutate(srtss_start = case_when(seasonality == "seasonal" ~ 5+4/12, 
         ...
         smc_file = case_when(grepl("param_mod", run) ~ "mod", 
                              TRUE ~ "0")) %>% 
  ungroup()
...
# system.time({out <- future_pmap(median_df[179,], run_sites, .progress = TRUE)})
