...
run_sites <- function(run, pfpr, vectors, seasonality, seasonality_coefs, draw, smc,
                      epi_pev, mass_pev, dose_5, coverage, smc_coverage, srtss_start, output_vars, 
                      smc_offset, smc_file){
  ...
  if(run %in% c("srtss_4_dose_param_mod_smc", "srtss_5_dose_param_mod_smc")){
    rtss_params <- c(" pev_Vmax 0.842998 pev_alpha 0.86805 pev_beta 70.9131")
  }else{
    rtss_params <- ""
  }
  ...
  smc     <- paste("smc", smc, "smc_start 5 smc_coverage", smc_coverage, "smc_age0 0.25 smc_age1 5")
  smc_add <- paste("num_smc 4 smc_offset", smc_offset)
  
  #-remaining model options and pasting together----------------------------
  options = paste("recalculate 2 num_people 60000 final_run 20 num_runs 3 itn 0 irs 0 output_type 0 drug_cov_0_0 0.26470592 drug_cov_1_0 0.2647059 drug_cov_2_0 0.0000000 drug_cov_3_0 0.0000000",
  ...
  mf_loc <- paste0("model_files/",smc_file,"/model_files")
  
  # Run the model
  name <- paste0(run, "_", seasonality, "_", pfpr, "_", coverage, "_", smc_coverage, "_", draw)
  exe  <- paste0("executable/helpVacc.exe")
  ...
  output$smc_coverage <- smc_coverage
  output$pfpr        <- pfpr 
