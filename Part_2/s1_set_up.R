...
output_vars <- function(lower_age = 0:25, upper_age = 1:26){
  #prev vars
...
#-coverage values--------------------------------------------------------------------------------
coverage <- 0.8 #seq(0.5, 0.9, 0.02)#c(0.8)#, 0.5, 0.9)
smc_coverage <- 0.75#seq(0.5, 0.9, 0.02)

#-create initial dataframe-----------------------------------------------------------------------
intervention_df <- crossing(run, pfpr, seasonality, draw, coverage) %>% mutate(smc = 1) %>% crossing(smc_coverage)
smc             <- data.frame(run = "smc") %>% crossing(pfpr, seasonality, draw, smc_coverage) %>% mutate(smc = 1, coverage=0)
baseline        <- data.frame(run = "baseline", coverage = 0) %>% crossing(pfpr, seasonality, draw) %>% mutate(smc = 0, smc_coverage=0)
...
#-run locally---------------------------------------------
final_df <- 
  left_join(param_df, df1, by=c("seasonality")) 

rm(output_vars, baseline, df1, intervention_df, param_df, smc, coverage, draw, draw_reduction, pfpr, run, seasonality)

source("S2_model_function")
system.time({out <- future_pmap(final_df, run_sites, .progress = TRUE)})
