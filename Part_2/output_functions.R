...
combine_outputs2 <- function(r, prev, seas, cov, smc_cov, dr){
  run           <- r       #baseline or smc
  pfpr          <- prev    #baseline pfpr levels
  seasonality   <- seas    #seasonality
  coverage      <- cov     #coverage value
  smc_coverage  <- smc_cov #smc coverage value
  draw          <- dr      #model parameter draw
  
  
  #-upload output in and select relevent variables---------------------------------------------------------------------- 
  raw <- read_csv(paste0(here::here(), "/output/", run, "_", seasonality, "_", pfpr, "_", coverage, "_", smc_coverage, "_", draw, ".csv")) 
  out <- raw
