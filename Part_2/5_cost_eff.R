...
figure_4_pd <-
  impact %>%
  filter(coverage %in% c(0.8,0), delivery_cost == 1.62) #smc and vaccine keeping
...
cpc1 <- cp_plot(x = filter(figure_4_pd, cost_per_dose == 2.69), title = "$2", ylab = "Cost per clinical case averted ($US)", ylimit = 150, icer_case)
cpc2 <- cp_plot(x = filter(figure_4_pd, cost_per_dose == 6.52), title = "$5", ylab = "Cost per clinical case averted ($US)", ylimit = 150, icer_case)
cpc3 <- cp_plot(x = filter(figure_4_pd, cost_per_dose == 12.91), title = "$10", ylab = "Cost per clinical case averted ($US)", ylimit = 150, icer_case)

cpd1 <- cp_plot_daly(x = filter(figure_4_pd, cost_per_dose == 2.69), title = "$2", ylab = "Cost per DALY averted ($US)", ylimit = 350, icer_daly)
cpd2 <- cp_plot_daly(x = filter(figure_4_pd, cost_per_dose == 6.52), title = "$5", ylab = "Cost per DALY averted ($US)", ylimit = 350, icer_daly)
cpd3 <- cp_plot_daly(x = filter(figure_4_pd, cost_per_dose == 12.91), title = "$10", ylab = "Cost per DALY averted ($US)", ylimit = 350, icer_daly)
...
cpc2 <- cp_plot(x = filter(figure_4_pd, cost_per_dose == 6.52), title = "$5", ylab = "Cost per clinical case averted ($US)", ylimit = 100, icer_case)
cpd2 <- cp_plot_daly(x = filter(figure_4_pd, cost_per_dose == 6.52), title = "$5", ylab = "Cost per DALY averted ($US)", ylimit = 210, icer_daly)
...
ggsave("cost_per_case_daly_averted_5.png", plog, height = 4.25, width = 10)
